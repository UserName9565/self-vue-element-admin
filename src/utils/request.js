...
service.interceptors.request.use(
  config => {
    ...
    // if(config.)
    var imfo = store.getters.imfo
    if (config.data) {
      // config.data.userId = imfo.userId;
      config.data.workAccount = imfo.account
      config.data.workDepartNumber = imfo.departNumber
      config.data.workChannelNo = imfo.userDefault1
    }
